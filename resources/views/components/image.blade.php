@if ($svg)
    <img decoding="async" src="{!! $svg !!}" alt="{!! $alt !!}" loading="lazy">
@else
    <picture>
        @php
        ...
        @if (!empty($imageElements))
            ...
            <img decoding="async" @if ($loading === 'eager') fetchpriority="high" @endif
                src="{!! $imageElements[0]['src'] !!}" alt="{!! $alt !!}" width="{!! $imageElements[0]['width'] !!}"
                height="{!! $imageElements[0]['height'] !!}" loading="{!! $loading !!}">
        @endif
    </picture>
@endif
